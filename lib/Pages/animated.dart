  ...
  void isertItem() {
   ...
   listKey.currentState!.insertItem(index);
    ...
    Future.delayed(const Duration(milliseconds:300),(){
      scrollController.animateTo(scrollController.position.viewportDimension, duration:const Duration(milliseconds: 300), curve: Curves.easeIn);
...
listKey.currentState!.removeItem(index, (context, animation) => SizeTransition(
     sizeFactor: animation,
     child: AnimatedListItem(text: removeItem, onPressed: (){},background: Colors.brown,)));
...
color: background,
        elevation: 5,
        child: ListTile(
          trailing: IconButton(onPressed: onPressed,icon:const Icon(Icons.delete ,color: Colors.brown,size: 23,),),

          title: Text(text,style: TextStyle(fontSize: 23,color: Colors.black),),
        ),
      ),
    );
  }
